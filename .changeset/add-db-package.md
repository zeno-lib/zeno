...
- `@zeno-lib/db` — `createSupabaseDrizzle({ schema, relations?, supabase?, connectionString? })` returns `{ admin, asUser, asUserTransaction, close }`. `admin` bypasses RLS. `asUser` is a chainable single-statement client that runs queries as the signed-in Supabase user (`db.asUser.select().from(t)`, `db.asUser.query.t.findMany()`): it records the query chain and replays it inside one RLS transaction after resolving verified claims from the bound Supabase client. `asUserTransaction(cb)` is the callback form for running several statements under one atomic RLS transaction. `relations` (from drizzle's `defineRelations`) enables the relational query API. Underlying Postgres pools are cached by imported schema object + connection config, so this ergonomic factory can be called with a request-scoped Supabase client.
