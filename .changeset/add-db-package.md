...
- `@zeno-lib/db/clients` — `createDrizzleClients({ schema, connectionString?, casing? })` returns `{ getDrizzleSupabaseAdminClient, getDrizzleSupabaseClient }`. Opens two `postgres-js` pools. `getDrizzleSupabaseAdminClient()` bypasses RLS. `getDrizzleSupabaseClient(accessToken)` decodes the JWT and returns `{ runTransaction }` — queries run inside `runTransaction` get `request.jwt.claims` (JSON) + `request.jwt.claim.sub` set and `set local role` (validated against `anon | authenticated | service_role`) so RLS policies apply.
