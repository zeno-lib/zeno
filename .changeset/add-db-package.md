...
- `@zeno-lib/db` — `createSupabaseDrizzle({ schema, relations?, supabase?, connectionString? })` returns `{ admin, rls, rlsTransaction, close }`. `admin` bypasses RLS. `rls` is a chainable single-statement RLS client (`db.rls.select().from(t)`, `db.rls.query.t.findMany()`) that records the query chain and replays it inside one RLS transaction after resolving verified claims from the bound Supabase client. `rlsTransaction(cb)` is the callback form for running several statements under one atomic RLS transaction. `relations` (from drizzle's `defineRelations`) enables the relational query API. Underlying Postgres pools are cached by imported schema object + connection config, so this ergonomic factory can be called with a request-scoped Supabase client.
