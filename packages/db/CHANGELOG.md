...
- a6f8ba4: Add `@zeno-lib/db/next`: `createRequestDb({ supabase, relations })` returns a React-`cache`d `getRequestContext()` / `getRequestDb()` that verify the session with `getClaims()` and bind an RLS client to the full claims (throwing the exported `UnauthenticatedError` when there is no verified `sub`, never falling back to `anon`), plus `defineAction(schema, (db, input, context) => …)`, which turns a `"use server"` export into one line. `react >=19` is a new optional peer dependency, needed only by this entry.
- 2c248ae: Add the query and error helpers Drizzle has no equivalent for, and two options on existing helpers.
...
- 6d8feb3: Rebuild on `prepack`, so the published tarball actually contains its `dist`.
...
- 25ba0ec: `@zeno-lib/db` ships compiled output instead of raw TypeScript.
...
- fb89503: `functionPolicies` takes a per-operation argument, a qualified function name,
