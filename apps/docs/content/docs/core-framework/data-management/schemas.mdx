...
Handler before querying through `createSupabaseDrizzle(...)` (RLS-scoped) or
`createAdminDrizzle(...)` (RLS bypass):
...
| `{project}-db/src/schema.ts` | Barrel re-exporting every table, passed as `schema` to `createSupabaseDrizzle(...)` / `createAdminDrizzle(...)`. |
